...
func TestPGQuestionRepository(t *testing.T) {
	...
	payload := questionDomain.Question{
		Content: "hello world",
	}
...
func TestPGQuestionRepository_PaginateQuestionsByCategory(t *testing.T) {
	...
	// 1. Buat kategori
	categorySchemaPayload := categoryRepository.CategorySchema{
		Name: "Matematika",
	}
	...
	question := questionDomain.Question{
		Content:    "2 + 2 = ?",
		Categories: []categoryDomain.Category{{ID: categorySchemaPayload.ID, Name: categorySchemaPayload.Name}},
	}
	created, err := questionRepo.Create(&question)
	assert.NoError(t, err)
	assert.NotNil(t, created)
...
func TestPGQuestionRepository_Update(t *testing.T) {
	...
	// Seed kategori awal
	cat1 := categoryRepository.CategorySchema{Name: "Math"}
	cat2 := categoryRepository.CategorySchema{Name: "Science"}
	cat3 := categoryRepository.CategorySchema{Name: "Physics"}
	...
	initialQuestion := QuestionSchema{
		...
		Hit:            0,
		Options:        []questionDomain.Option{{Content: "A", IsCorrect: false}},
		ExplanationURL: "http://original.com",
		Categories:     []categoryRepository.CategorySchema{cat1, cat2}, // Start dengan 2 kategori
		CreatedAt:      time.Now(),
		UpdatedAt:      time.Now(),
	}
	...
	var initialState QuestionSchema
	...
	updatedQuestion := questionDomain.Question{
		ID:             created.ID,
		Content:        "Updated Content",
		Hit:            5,
		Options:        []questionDomain.Option{{Content: "B", IsCorrect: true}},
...
func TestPGQuestionRepository_GetRandomByCategories(t *testing.T) {
	...
	// 1. Buat kategori
	catMath := categoryRepository.CategorySchema{Name: "Math"}
	catScience := categoryRepository.CategorySchema{Name: "Science"}
	...
	q1 := &questionDomain.Question{
		Content:    "2 + 2 = ?",
		Categories: []categoryDomain.Category{{ID: catMath.ID, Name: catMath.Name}},
	}
	q2 := &questionDomain.Question{
		Content:    "Water boils at 100C?",
		Categories: []categoryDomain.Category{{ID: catScience.ID, Name: catScience.Name}},
	}
