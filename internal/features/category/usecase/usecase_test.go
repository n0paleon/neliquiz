...
import (
	...
	"NeliQuiz/internal/features/category/repository/mocks"
	"github.com/stretchr/testify/assert"
	"testing"
)

func TestUseCase_GetListCategories(t *testing.T) {
	...
	t.Run("should return all categories when query is empty", func(t *testing.T) {
		categoryRepo.On("FindAll").Return([]domain.Category{
			{ID: "1", Name: "Math"},
			{ID: "2", Name: "Science"},
		}, nil).Once()
	...
	t.Run("should return search result when query and limit are provided", func(t *testing.T) {
		query := "ma"
		limit := 10
		categoryRepo.On("SearchCategoriesByName", query, limit).Return([]domain.Category{
	...
	t.Run("should propagate error from repository", func(t *testing.T) {
		...
		categoryRepo.On("SearchCategoriesByName", query, limit).Return([]domain.Category(nil), mockErr).Once()
